...
some_columns = comb[['fitness','vel','dvel','joint_count','extremity_count','nparts']].copy()
#print(comb.describe().to_string())
#comb = comb.drop(columns=['run_y', 'id'])
#print(sys.argv[1])
#print(comb.describe().to_string())
#print(some_columns.describe())
#print(comb["robot_id"])
...
for measure in measures:
    ...
    #print(sm.GLM(comb[measure].astype(float), comb["fitness_function"], family=sm.families.Poisson()).fit().summary());
    #md_robots = smf.mixedlm((measure +" ~ fitness_function+world"), comb, groups=comb["fitness_function"])
    for fitness_function in ["old_fitness", "divided_size", "diagonal", "bbox"]:
        print("Fitness function base", fitness_function)
        print(smf.glm(formula=(measure +" ~ C(fitness_function, Treatment('"+ fitness_function +"'))"), data=comb, groups=comb["fitness_function"],  family=sm.families.Poisson()).fit().summary())
    #mdf_robots = md_robots.fit()
