...
if len(sys.argv) <= 1:
    print "need to specify experiments source folder"
    sys.exit(1)
...
# merge the two dataframes for each experiment
###############################################################################
#df0=data_lists['world1'][0]
#df1=data_lists['world1'][1]

#need to merge on robot id
#discard lines in seond frame that don't have id in first
#drop run column from second frame


#check if merge OK.

#world0 now contains some duplicate run ids. could they be renamed easily?


#%%############################################################################
# could cut to have an equal number of runs
###############################################################################
...
f, axarr = plt.subplots(len(measures),3)
nbins=30
coord=0
for m in measures: #one plot for each measure
    ...
    for exp in last_gens.keys(): #combine plots for each experiment
        ...
        axarr[coord,0].hist(plot_gens, int(nbins), m_range, alpha=0.5, label=exp, normed=1) #normal histogram
        axarr[coord,1].hist(plot_gens, int(nbins), m_range, label=exp, normed=1, histtype="step", cumulative=True) #CDF
...
#plt.show()
f.set_size_inches(20,30) #arbitrary paper size
plt.savefig("measures_hist_cdf_box.pdf")
